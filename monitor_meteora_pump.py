...
def load_active_strategies():
    ...
    try:
        ...
        for row in rs.rows:
            # 兼容处理：确保解析出 dict 格式
            raw_param = row[2]
            params = json.loads(raw_param) if isinstance(raw_param, str) else raw_param
            strategies.append({
                "config_id": row[0],
                "config_name": row[1],
                "params": params or {}
...
def fetch_tokens_by_strategy(strategy: dict):
    ...
    币龄逻辑：age_hours = 当前 - created_at
        min_age_hours：币至少存活多少小时（不能太新）
        max_age_hours：币最多存活多少小时（不能太老）
    """
    client = get_db_client()
    if not client:
        return []
    ...
    # ✅ 修复SQL时间条件：
    # julianday('now') - julianday(created_at) >= min_age_hours  币龄 >=最小小时
    # julianday('now') - julianday(created_at) <= max_age_hours 币龄 <=最大小时
    query = """
    SELECT
        address,
        symbol,
...
def run_pump_strategy_monitor():
    ...
    for strat in strategies:
        ...
        if tokens:
            ...
            for t in tokens[:5]:  # 推送前5个标的，避免消息过长
